...
def get_api_key():
    """Get the API key from environment, raise error if not found."""
    api_key = os.getenv("OBSIDIAN_API_KEY", "")
    if api_key == "":
        raise ValueError(f"OBSIDIAN_API_KEY environment variable required. Working directory: {os.getcwd()}")
    return api_key

def get_obsidian_host():
    """Get the Obsidian host from environment."""
    return os.getenv("OBSIDIAN_HOST", "127.0.0.1")
